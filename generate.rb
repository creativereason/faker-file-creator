...
class GenerateNames
  def init
    row_header = "first_name\tlast_name\tcompany_name\tsales\taddress_1\taddress_2\tcity\tstate\tzip\tphone\tfax\n"
    ...
    for i in 0..1000 do
        ...
        if i % 9 == 0
          a.address_2 = Faker::Address.secondary_address 
        end
        # a.address_2 = Faker::
...
class CompanyContact
  attr_accessor :first_name, :last_name, :company_name, :address_1, :address_2, :city, :state, :zip, :phone, :fax
 ...
 def to_s 
  "#{first_name}\t#{last_name}\t#{company_name}\t$#{sales}\t#{address_1}\t#{address_2}\t#{city}\t#{state}\t#{zip}\t#{phone}\t#{fax}\n"
 end
